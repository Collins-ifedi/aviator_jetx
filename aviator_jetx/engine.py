...
class CrashGameEngine:
    ...
    def _check_exposure_risk(self, current_multiplier: Decimal) -> bool:
        ...
        Returns True if we MUST crash now to save money.
        """
        if not self._round:
            return False
        ...
        for bet in self._round.bets.values():
            if not bet.cashed_out:
                # Calculate what we would pay if they cashed out NOW
                current_exposure += bet.amount * current_multiplier

        if current_exposure > GameConfig.MAX_EXPOSURE_LIMIT:
            return True
    ...
    async def get_current_state(self) -> Dict:
        ...
        async with self._lock:
            ...
            if self._round.state == GameState.FLYING:
                ...
                # 2. Check Risk Limit (Bankruptcy Protection)
                # If current payouts exceed safe limit, force crash immediately
                ...
                # 4. Minimum Duration (Visual Buffer)
                # Ensure 1.01x still flies for a split second for UI rendering
                ...
                if (has_reached_target or has_reached_cap or force_crash) and min_time_passed:
                    ...
                    # If we forced a crash due to risk or cap, update the official crash point
                    if force_crash or has_reached_cap:
                        self._round.crash_point = min(current_mult, GameConfig.MAX_MULTIPLIER_CAP)
                    else:
                        # Otherwise clamp result to the pre-generated point
