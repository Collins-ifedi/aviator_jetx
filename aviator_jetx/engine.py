...
class CrashGameEngine:
    ...
    def _generate_crash_point(self) -> Decimal:
        ...
        # 2. House Edge Check (Instant Loss)
        if rand < float(GameConfig.HOUSE_EDGE):
            return GameConfig.MIN_CRASH

        # 3. Distribution Curve
        # The formula 1 / (1 - rand) creates a "long tail" distribution.
        # We adjust it to fit our desired volatility.
        # Normalizing logic: (0.99 / (1 - rand)) roughly maps 0.5 -> 2.0x
        try:
    ...
    async def start_new_round(self) -> Dict:
        """
        Initialize a round. 
        Transitions: IDLE -> BETTING -> FLYING (Immediately for this setup)
        """
        async with self._lock:
            # Prevent starting over an active round
            ...
            # --- IMMEDIATE START (Zero Waiting) ---
            # In a real lobby, you might wait 5 seconds here. 
            # For this seamless version, we jump straight to flight.
    ...
    async def get_current_state(self) -> Dict:
        ...
        async with self._lock:
            ...
            return {
                ...
                # Only reveal crash point if crashed
    ...
    async def place_bet(
        ...
        async with self._lock:
            ...
            # Allow bets if:
            # 1. Game is in BETTING state (pre-flight)
    ...
    async def cashout(self, user_id: int, client_observed_multiplier: float) -> float:
        ...
        async with self._lock:
            ...
            # If server says crashed, it's over. No grace period for cashouts.
