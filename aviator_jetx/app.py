...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # The 'web_app' argument triggers the popup within Telegram
    keyboard = [
        [
            InlineKeyboardButton(
                text="🚀 START FLYING", 
...
async def lifespan(app: FastAPI):
    ...
    if BOT_TOKEN:
        ...
        # 'allowed_updates=Update.ALL_TYPES' ensures we catch all relevant events
        # 'secret_token' adds a layer of security to verify requests come from Telegram
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        # Always return 200 to Telegram to prevent them from retrying bad updates endlessly
        return Response(status_code=200)
...
async def state_error_handler(_, exc: StateError):
    """Handles engine state errors (e.g., betting when flying)"""
    return JSONResponse(status_code=409, content={"detail": str(exc)})

@app.exception_handler(BetError)
async def bet_error_handler(_, exc: BetError):
    """Handles betting logic errors (e.g., insufficient funds in logic)"""
...
async def api_start_round(payload: StartRoundRequest):
    ...
    Ideally called by a background worker, but exposed here for the demo architecture.
    """
    try:
        return await engine.start_new_round(payload.client_seed)
    except Exception:
        # If round already active, just return state
        return await engine.get_current_state()

@app.get("/api/state")
async def api_state():
    """Get current game engine state (High frequency polling)."""
    return await engine.get_current_state()
...
async def api_cashout(payload: CashoutRequest, session: AsyncSession = Depends(get_session)):
    ...
    # 2. Credit DB
    user = await get_or_create_user(session, payload.user_id)
    # We trust the engine's payout calculation which handles the math
