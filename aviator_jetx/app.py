...
- Serving the Web App
- BACKGROUND GAME LOOP (Drivers the engine)
"""
...
async def run_game_loop():
    ...
    - CRASHED: Waits for cooldown, then resets.
    """
    ...
    while True:
        try:
            ...
            if status == "OFFLINE" or status == "IDLE":
                ...
                # We just poll to keep the loop alive.
                await asyncio.sleep(0.1)

            elif status == "CRASHED":
                # Game over. Wait 3 seconds so users see the crash result.
                await asyncio.sleep(3)
                # Start fresh round
                await engine.start_new_round()
                logger.info("Game Loop: Restarting after crash...")
...
async def api_place_bet(payload: BetRequest, session: AsyncSession = Depends(get_session)):
    ...
    CRITICAL CHANGE: Calls trigger_flight() to start the game.
    """
